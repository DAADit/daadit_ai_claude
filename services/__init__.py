...
try:
    if llm_api_patch.patch_llm_api_service():
        _log.info(
            "daadit_ai_claude.services: LLMApiService patched at import time"
        )
    else:
        _log.info(
            "daadit_ai_claude.services: LLMApiService not yet importable "
            "at services/__init__.py time — will retry from _register_hook "
            "and _get_provider"
        )
except Exception:  # noqa: BLE001
    _log.exception(
        "daadit_ai_claude.services: import-time LLMApiService patch raised"
    )
