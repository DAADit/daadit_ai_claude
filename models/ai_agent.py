...
class AIAgent(models.Model):
    ...
    # ------------------------------------------------------------------ #
    # Diagnostic introspection — find which methods on the merged       #
    # ai.agent class raise our target error markers, so we can verify   #
    # the registry_patches scan caught them. Call via:                  #
    #     odoo_execute_kw(model="ai.agent",                              #
    #         method="daadit_claude_debug_introspect",                  #
    #         args=[], kwargs={}, allow_mutations=True)                  #
    # ------------------------------------------------------------------ #
    @api.model
    def daadit_claude_debug_introspect(self):
        from ..services.registry_patches import (
            ERROR_MARKERS,
            _scan_code_for_markers,
        )
        import inspect
        import sys
        out = {"ai.agent": [], "ai.embedding": [], "modules": []}
        # ---- 1. Class-MRO scan (ai.agent + ai.embedding) ----
        for model_name in ("ai.agent", "ai.embedding"):
                ...
                for attr_name, attr_value in vars(base).items():
                    ...
                    if _scan_code_for_markers(code, ERROR_MARKERS):
                        ...
                        out[model_name].append({
                            "method": attr_name,
                            "defined_in": f"{base_module}.{base.__name__}",
                            "signature": sig,
                            "wrapped": bool(
                                getattr(attr_value, "_daadit_claude_patch", False)
                                or getattr(attr_value, "_daadit_patch", False)
                            ),
                        })
        # ---- 2. Global module scan (free functions / unrelated classes) ----
        for module_name, module in list(sys.modules.items()):
            if module is None:
                continue
            if not module_name or "odoo.addons.ai" not in module_name:
                continue
            if "daadit_ai_claude" in module_name:
                continue
            try:
                    ...
                    if callable(attr) and not isinstance(attr, type):
                        ...
                        if _scan_code_for_markers(code, ERROR_MARKERS):
                            out["modules"].append({
                                ...
                                "kind": "function",
                                "wrapped": bool(
                                    getattr(attr, "_daadit_claude_patch", False)
                                    or getattr(attr, "_daadit_patch", False)
                                ),
                            })
                        ...
                        for method_name, method in vars(attr).items():
                            ...
                            if _scan_code_for_markers(code, ERROR_MARKERS):
                                out["modules"].append({
                                    ...
                                    "kind": "method",
                                    "wrapped": bool(
                                        getattr(method, "_daadit_claude_patch", False)
                                        or getattr(method, "_daadit_patch", False)
                                    ),
                                })
                except Exception:  # noqa: BLE001
                    continue
